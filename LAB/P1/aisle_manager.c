...
unsigned short get_section(unsigned long* aisle, int index) {
  ...
  short section = 0;
  unsigned long temp = *aisle;
  section = (temp >> (index*16)) & SECTION_MASK;
...
unsigned short get_spaces(unsigned long* aisle, int index) {
  // TODO: implement this method
  unsigned long temp = get_section(aisle, index);
  short res = temp & SPACES_MASK; 
...
unsigned short get_id(unsigned long* aisle, int index) {
  // TODO: implement this method
  unsigned short temp = get_section(aisle, index);
  unsigned short ID = temp & ID_MASK;
  unsigned short res = ID >> 10;
  return res;
}
...
void set_section(unsigned long* aisle, int index, unsigned short new_section) {
  // TODO: implement this method
  unsigned long temp = *aisle;
  unsigned long mask =((unsigned long) 0xFFFF) << (index * 16);
  new_section = ~new_section;
  unsigned long section = ((unsigned long)(new_section)) << (index*16);
  temp  = (temp | mask);
  *aisle = temp & (~section);
...
void set_spaces(unsigned long* aisle, int index, unsigned short new_spaces) {
  // TODO: implement this method
  if (ID_MASK & new_spaces){ 
    return;
  }	  
  unsigned short temp = get_section(aisle, index);
  unsigned short new = (temp & ID_MASK)|new_spaces;
  set_section(aisle, index, new);
...
void set_id(unsigned long* aisle, int index, unsigned short new_id) {
  ...
  unsigned short temp = get_section(aisle, index);
  unsigned short new = (temp & 0x03FF)| (new_id << 10);
  set_section(aisle, index, new);
...
void toggle_space(unsigned long* aisle, int index, int space_index) {
  ...
  unsigned short shiftmask = (unsigned short)0x01 << space_index;
  unsigned short space_position = space & shiftmask;
  space = space | shiftmask;
  unsigned short new = space & (~space_position);
...
void rotate_items_left(unsigned long* aisle, int index, int n) {
  ...
  unsigned short space = get_spaces(aisle, index);
  int remainder = n % 10;
  unsigned short fix = space << remainder;
  unsigned short move = space >> (10 - remainder);
  unsigned short res = 0b1111111111 & (fix | move);
  set_spaces(aisle, index, res); 
}

...
void* mm_malloc(size_t size) {
 ...
 /*Get more heap space of size at least reqSize if the existed free block size is smaller than reaSize */
  if (!ptrFreeBlock) {
  ...
  /* Consider when the free block size is larger enough, split it into an allocated block and another free block*/
  if ((blockSize - reqSize) > MIN_BLOCK_SIZE) {
    ((BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize))->sizeAndTags = (size_t)((blockSize - reqSize) | TAG_PRECEDING_USED) & ~TAG_USED;
    *((size_t*)UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize - WORD_SIZE)) =(size_t)((blockSize - reqSize) | TAG_PRECEDING_USED) & ~TAG_USED;
    
    /*Set block tag to preceding used and used*/
    ptrFreeBlock->sizeAndTags = (reqSize | (ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED)) | TAG_USED;

    insertFreeBlock((BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize));

  } else {
    /*If the free block is exactly enough the reqSize block, check the following free block */	  
    ptrFreeBlock->sizeAndTags |= TAG_USED;
    /*Set the next of previous is used*/   
    ((BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize))->sizeAndTags |= TAG_PRECEDING_USED;
  ...
  /*Return ptr from the head of payload*/
...
void mm_free(void* ptr) {
  ...
  *(size_t*)UNSCALED_POINTER_ADD(blockInfo, payloadSize) = blockInfo->sizeAndTags;
  ...
  /*Set following block footer if it's free*/
  if ((followingBlock->sizeAndTags) & (TAG_USED == 0)) {
    followingpayloadSize = SIZE((followingBlock->sizeAndTags) - WORD_SIZE);  
    *(size_t*)UNSCALED_POINTER_ADD(followingBlock, followingpayloadSize) &= ~TAG_PRECEDING_USED;
  }
